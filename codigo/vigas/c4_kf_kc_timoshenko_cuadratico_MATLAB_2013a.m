...
x = simple(N1*x1 + N2*x2 + N3*x3);
%w = simple(N1*w1 + N2*w2 + N3*w3);
%t = simple(N1*t1 + N2*t2 + N3*t3);
...
disp('Bf = (2/L) * '),    pretty(simple(Bf/(2/L)))
disp('Bc = (2/L) * '),    pretty(simple(Bc/(2/L)))
...
Kf = simple(subs(Bf.'*E*I*Bf*L/2,   xi,xi1)*w1 + ...
            subs(Bf.'*E*I*Bf*L/2,   xi,xi2)*w2) ;
         
Kc = simple(subs(Bc.'*G*Aast*Bc*L/2,xi,xi1)*w1 + ...
