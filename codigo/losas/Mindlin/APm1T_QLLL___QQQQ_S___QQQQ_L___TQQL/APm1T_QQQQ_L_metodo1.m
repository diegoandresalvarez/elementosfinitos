...
gpg = [ crear_gpg('xi',  idx(XI, :))    % Los + = gxi1  gxi2  gxi3  gxi4   gxi5   gxi6
        crear_gpg('eta', idx(ETA,:)) ]; % Los x = geta7 geta8 geta9 geta10 geta11 geta12
...
for i = [ XI ETA ]
   ...
   switch i
      case XI
         A         = [ ones(ngamma_1_dir,1) xxi eeta  xxi.*eeta  eeta.^2  xxi.*eeta.^2];
         variables = [ 1  xi  eta  xi*eta  eta^2  xi*eta^2 ];
      case ETA
         A         = [ ones(ngamma_1_dir,1) xxi eeta  xxi.*eeta  xxi.^2 eeta.*xxi.^2];
...
ngamma = max(max(idx));
gpg = [ crear_gpg('xi',  1:ngamma)    % Los + = gxi1  .. gxi12
        crear_gpg('eta', 1:ngamma) ]; % Los x = geta1 .. geta12
gpg = gpg(:).';  % gxi1 geta1 gxi2 geta2 .. gxi12 geta12
