...
% se convierten a coordenadas locales
c = cosd(theta); s = sind(theta);
b1_loc =  b1_glob.*s.*c + q1_glob.*s.*c;
b2_loc =  b2_glob.*s.*c + q2_glob.*s.*c;
q1_loc = -b1_glob.*s.*s + q1_glob.*c.*c;
q2_loc = -b2_glob.*s.*s + q2_glob.*c.*c;

%% separo la memoria
K   = zeros(ngdl);    % matriz de rigidez global
f   = zeros(ngdl,1);  % vector de fuerzas nodales equivalentes global
Ke  = cell(nb,1);     % matriz de rigidez local para cada EF en coordenadas globales 
fe  = cell(nb,1);     % vector de fuerzas nodales equivalentes para cada EF en coordenadas globales
T   = cell(nb,1);     % matriz de transformacion de coordenadas para cada EF 
...
for e = 1:nb  % para cada barra
   ...
   % vector de fuerzas nodales equivalentes y matriz de rigidez local 
   % expresada en el sistema de coordenadas locales para la barra e
   floc = fe_ec_dif(long(e), E, A(e), I(e), b1_loc(e),b2_loc(e), q1_loc(e),q2_loc(e));
   Kloc = Ke_ec_dif(long(e), E, A(e), I(e));
...
fe2{1} = [0     -5.60  -3.733  0     -5.60   +3.733 ]'; % OJO con los signos
fe2{2} = [0      0      0      0      0      0      ]'; % mirar pag 613
fe2{3} = [0      0      0      0      0      0      ]';
%}
