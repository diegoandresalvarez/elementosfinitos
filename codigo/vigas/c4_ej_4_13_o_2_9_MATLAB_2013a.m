...
disp('Kf = (E*I)/(3*L) *'); disp(simple(Kf/((E*I)/(3*L))))       
         
         
Kc = simple(subs(Bc_sustitutiva.'*G*Aast*Bc_sustitutiva*L/2,xi,xi1)*w1 + ...
            subs(Bc_sustitutiva.'*G*Aast*Bc_sustitutiva*L/2,xi,xi2)*w2);

disp('Kc = (G*Aast)/(9*L) *'); disp(simple(Kc/((G*Aast)/(9*L))))         
