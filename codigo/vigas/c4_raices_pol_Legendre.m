...
%% Gauss-Legendre de orden n, un polinimio de grado n y otro de grado n-1
...
inter = solve(f-g,... % = 0  % calculo la interseccion de ambos polinomios
              x)
       
figure; hold on; grid on        % grafico la respuesta
ezplot(f,[-1 1]); ezplot(g,[-1 1]);
gg = inline(vectorize(char(g)),'x');
