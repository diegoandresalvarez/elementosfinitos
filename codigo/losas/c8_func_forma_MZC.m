...
B = simplify([BB{1} BB{2} BB{3} BB{4}]);
...
K = simplify(int(int(B.'*Df*B*det_J, xi, -1, 1), eta, -1, 1))
toc
...
M = simplify(int(int(rho*NN_.'*NN_*det_J,xi,-1,1),eta,-1,1));
disp('M = (a*b*rho)*');
disp(M/(a*b*rho))
...
y
Df_B_simplify = simplify(Df_B*4/DD)
Df_B = DD/4*[...  % = Df*B
