...
Kf = simplify(subs(Bf.'*E*I*Bf*L/2,   xi,-sym(sqrt(1/3)))*w + subs(Bf.'*E*I*Bf*L/2,   xi,sym(sqrt(1/3)))*w);
Kc = simplify(subs(Bc.'*G*Aast*Bc*L/2,xi,-sym(sqrt(1/3)))*w + subs(Bc.'*G*Aast*Bc*L/2,xi,sym(sqrt(1/3)))*w);
...
%% Evaluo la curvatura
chi = simplify((2/L)*(diff(N1,xi)*t1 + diff(N2,xi)*t2));
disp('CHI = '), pretty(chi)
