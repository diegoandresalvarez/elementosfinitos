...
for e = 1:nef
   fill3(xnod(LaG(e,[1 2 3 4 1]),X), ...
         ...
         1000*xdef(LaG(e,[1 2 3 4 1]),ww),...
         1000*xdef(LaG(e,[1 2 3 4 1]),ww)); %deformada
end
daspect([1 1 2000/(ESC_W)]);
axis tight
title('Deformada [mm]','FontSize',20)
xlabel(['Eje X [' U_LONG ']']);
ylabel(['Eje Y [' U_LONG ']']);
plot([0 2 2 0 0],[0 0 4 4 0],'k','LineWidth',2); % borde del apoyo
%view(3)
caz = -67.312481338394448;
cel =  11.369332805396519;
view(caz,cel);
print(['w.png'],'-dpng','-r200');
...
figure
%subplot(1,3,1); 
stem3(xnod(:,X), xnod(:,Y), 1000*mat_rea(:,ww), 'filled');
...
plot_M_or_Q({ 1000*Mx,  ['Momentos Mx [' unitsM ']'] })
print(['Mx.png'],'-dpng','-r200');

plot_M_or_Q({ 1000*My,  ['Momentos My [' unitsM ']'] })
print(['My.png'],'-dpng','-r200');

plot_M_or_Q({ 1000*Mxy, ['Momentos Mxy [' unitsM ']'] })
print(['Mxy.png'],'-dpng','-r200');
...
print(['Mf1.png'],'-dpng','-r200');

plot_M_or_Q({ Mf2_xy, ['Mf2_{xy} [' unitsM ']'], { ang+pi/2 } })
print(['Mf2.png'],'-dpng','-r200');

plot_M_or_Q({ Mt_max, ['Mt_{max} [' unitsM ']'], { ang+pi/4, ang-pi/4 } })
print(['Mtmax.png'],'-dpng','-r200');
...
print(['Qx.png'],'-dpng','-r200');

plot_M_or_Q({ Qy,    ['Cortantes Qy [' unitsQ ']'],      { }     })
print(['Qy.png'],'-dpng','-r200');

plot_M_or_Q({ Q_max, ['Cortantes Q_{max} [' unitsQ ']'], { ang } })
print(['Qmax.png'],'-dpng','-r200');
        ...
        if vars == 3
            ...
            for j = 1:length(angulos)
                ...
                quiver(xnod(:,X),xnod(:,Y),...             
                    ...
                    esc,'k', 'ShowArrowHead','off', 'Marker','.');                    
            end
        end
        xlabel(['Eje X [' U_LONG ']']);
        ylabel(['Eje Y [' U_LONG ']']);
